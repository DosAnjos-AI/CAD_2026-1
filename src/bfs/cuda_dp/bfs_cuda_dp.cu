...
/* Executa o laço BFS-DP sobre os buffers de fila fornecidos */
static void executar_bfs_dp(int N, int *d_adj, int *d_offset, int *d_size,
                              unsigned int *d_distance, int *d_parent,
    ...
    while (queueSize > 0) {
        ...
        kernel_bfs_principal<<<num_blocos, 256>>>(
            ...
            d_distance, d_parent, queueSize, curQ, nxtQ, d_nextQueueSize
        );
...
int main(int argc, char *argv[]) {
    ...
    /* warm-up: reseta estado e executa BFS-DP completo para validar */
    executar_bfs_dp(N, d_adj, d_offset, d_size, d_distance, d_parent,
    ...
    printf("bfs,cuda_dp,%dx%d,%d,%.6f,%s\n", N, M, runs, tempo_total, corretude);

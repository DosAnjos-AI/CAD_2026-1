 ...
 * Mesma lógica do kernel principal — 1 aresta por thread com atomicMin.
 * NÃO lança kernels adicionais (não recursivo, conforme artigo).
 */
__global__ void kernel_sssp_secundario(int M,
                                        int *d_origem, int *d_destino,
                                        int *d_custo, int *d_dist,
                                        int *d_preNode, int *d_finished) {
...
/*
 * Kernel principal do Bellman-Ford com Dynamic Parallelism.
 * Quando thread 0 global detecta uma atualização de distância,
 * lança kernel_sssp_secundario via DP para propagar as relaxações
 * sem aguardar a próxima iteração do laço externo na CPU.
 */
__global__ void kernel_sssp_principal(int M,
                                       int *d_origem, int *d_destino,
                                       int *d_custo, int *d_dist,
                                       int *d_preNode, int *d_finished) {
    int thread_id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (d_dist[u] != INT_MAX && d_dist[u] + w < d_dist[v]) {
        ...
        *d_finished  = 0;
        ...
        /* Thread 0 global lança kernel secundário para antecipar relaxações */
        if (thread_id == 0) {
            cudaStream_t s;
            cudaStreamCreateWithFlags(&s, cudaStreamNonBlocking);
            int num_blocos = (M + 255) / 256;
            kernel_sssp_secundario<<<num_blocos, 256, 0, s>>>(
                M, d_origem, d_destino, d_custo, d_dist, d_preNode, d_finished);
            cudaStreamDestroy(s);
        }
    }
}
...
/* Executa laço Bellman-Ford DP com convergência antecipada */
static void executar_sssp_dp(int N, int M, int *d_origem, int *d_destino, int *d_custo,
    ...
    for (int iter = 0; iter < N - 1; iter++) {
        ...
        kernel_sssp_principal<<<num_blocos, 256>>>(
            M, d_origem, d_destino, d_custo, d_dist, d_preNode, d_finished);
...
int main(int argc, char *argv[]) {
    ...
    /* warm-up: reseta d_dist e executa SSSP-DP completo para validar */
    executar_sssp_dp(N, M, d_origem, d_destino, d_custo,
    ...
    printf("sssp,cuda_dp,%dx%d,%d,%.6f,%s\n", N, M, runs, tempo_total, corretude);

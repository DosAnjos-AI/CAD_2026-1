/* sssp_otimizado_openmp.c
 * Versao otimizada do Algoritmo 2 de Nogueira et al. (SSCAD 2024).
 * OPT-1: #pragma omp atomic write em finished.
 * OPT-2: #pragma omp parallel for schedule(dynamic, 512) no relaxamento.
 * Uso: ./sssp_otimizado_openmp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- sssp otimizado (OPT-1 + OPT-2) ---------- */

static void sssp_openmp(int N, int numEdges, int *origens, int *destinos, int *custos,
                         int *dist) {
    ...
    while (!finished) {
        finished = 1;
        /* OPT-2: parallel for com escalonamento dinamico no lugar da distribuicao manual */
        ...
        for (int i = 0; i < numEdges; i++) {
            ...
            if (dist[origem] != INT_MAX && dist[origem] + custo < dist[destino]) {
                ...
                /* OPT-1: escrita atomica em finished para evitar corrida de dados */
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        sssp_openmp(N, E, origens, destinos, custos, dist);
        ...
        printf("[TESTE] sssp openmp otimizado N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("sssp", "openmp", "otimizado", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "sssp openmp otimizado %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL sssp openmp otimizado N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio sssp openmp otimizado hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

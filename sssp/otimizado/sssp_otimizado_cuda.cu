/* sssp_otimizado_cuda.cu
 * Versao otimizada do Algoritmo 1 de Nogueira et al. (SSCAD 2024).
 * OPT-1: dimensionamento dinamico (numThreadsPerBlock=128) e distribuicao
 * correta de arestas por thread (arestas_por_thread), eliminando o erro
 * do artigo onde startId = threadId * numEdges deixava apenas thread 0 ativa.
 * Uso: ./sssp_otimizado_cuda [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- kernel SSSP otimizado (OPT-1) ---------- */

/* Distribuicao correta: cada thread recebe arestas_por_thread arestas contíguas */
__global__ void sssp_kernel(int numEdges, int *origens, int *destinos, int *custos,
                             int *dist, int *preNode, int *finished) {
    int threadId           = blockIdx.x * blockDim.x + threadIdx.x;
    int numThreads         = gridDim.x * blockDim.x;
    int arestas_por_thread = (numEdges + numThreads - 1) / numThreads;

    int startId = threadId * arestas_por_thread;
    int endId   = startId + arestas_por_thread;
...
/* numBlocks calculado uma vez fora do loop (OPT-1) */
static void sssp_cuda(int N, int numEdges, int *origens, int *destinos, int *custos,
                       int *dist, int *preNode, int *d_finished) {
    const int numThreadsPerBlock = 128;
    int numBlocks = (numEdges + numThreadsPerBlock - 1) / numThreadsPerBlock;

    int finished = 0;
    while (!finished) {
        *d_finished = 1;
        sssp_kernel<<<numBlocks, numThreadsPerBlock>>>(numEdges, origens, destinos,
                                                        custos, dist, preNode, d_finished);
...
static void modo_teste(void) {
    ...
    int *origens, *destinos, *custos, *dist, *preNode, *d_finished;
    ...
    cudaMallocManaged(&d_finished, sizeof(int));
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        sssp_cuda(N, E, origens, destinos, custos, dist, preNode, d_finished);
        ...
        printf("[TESTE] sssp otimizado cuda N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
    ...
    cudaFree(dist); cudaFree(preNode); cudaFree(d_finished);
    free(dist_ref);
}
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("sssp", "cuda", "otimizado", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "sssp otimizado cuda %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL sssp otimizado cuda N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio sssp otimizado cuda hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

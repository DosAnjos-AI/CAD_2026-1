/* sssp_artigo_cuda_dp.cu
 * Implementacao fiel ao Algoritmo 4 do artigo de Nogueira et al. (SSCAD 2024).
 ...
 * processa arestas (ver comentario em sssp_artigo_cuda.cu). Fiel ao Alg. 4.
...
/* ---------- kernels SSSP com Dynamic Parallelism (Algoritmo 4) ---------- */
...
__global__ void sssp_kernel_dp(int numEdges, int *origens, int *destinos,
    ...
    /* thread 0 do bloco 0 lanca kernel secundario se houve mudanca */
    if (mudou && threadIdx.x == 0 && blockIdx.x == 0) {
        *finished = 0;
        sssp_kernel_secundario<<<1, 1>>>(numEdges, origens, destinos,
                                          custos, dist, preNode);
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] sssp cuda_dp N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("sssp", "cuda_dp", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "sssp cuda_dp %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL sssp cuda_dp N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio sssp cuda_dp hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

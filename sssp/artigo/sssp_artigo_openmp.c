/* sssp_artigo_openmp.c
 * Implementacao fiel ao Algoritmo 2 do apendice de Nogueira et al. (SSCAD 2024).
 * Bellman-Ford paralelo com #pragma omp parallel; representacao por lista de arestas.
 * Uso: ./sssp_artigo_openmp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void sssp_openmp(int N, int numEdges, int *origens, int *destinos, int *custos,
    ...
    while (!finished) {
        ...
        #pragma omp parallel
        {
            ...
            int inicio            = threadId * numEdgesPerThread;
            int fim               = (threadId + 1) * numEdgesPerThread;

            if (inicio >= numEdges) inicio = numEdges;
            else if (fim >= numEdges) fim   = numEdges;

            for (int i = inicio; i < fim; i++) {
                int origem  = origens[i];
                int destino = destinos[i];
                int custo   = custos[i];
                /* escrita idempotente em finished: fiel ao Alg. 2 */
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] sssp openmp N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("sssp", "openmp", "artigo", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "sssp openmp %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL sssp openmp N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio sssp openmp hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

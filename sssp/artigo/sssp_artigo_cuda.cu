/* sssp_artigo_cuda.cu
 * Implementacao fiel ao Algoritmo 1 do apendice de Nogueira et al. (SSCAD 2024).
 * Bellman-Ford CUDA; lista de arestas; memoria unificada (cudaMallocManaged).
 * NOTA: startId = threadId * numEdges conforme pseudocódigo — cada thread recebe
 * bloco iniciando em threadId * numEdges, logo apenas thread 0 processa arestas.
 * Isso é fiel ao Alg. 1 do apendice, que provavelmente contém erro tipografico
 * (deveria ser numEdgesPerThread). Script A implementa exatamente como descrito.
 * Uso: ./sssp_artigo_cuda [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- kernel SSSP (Algoritmo 1) ---------- */

/* startId = threadId * numEdges: fiel ao Alg. 1; apenas thread 0 processa arestas */
...
static void sssp_cuda(int N, int numEdges, int *origens, int *destinos, int *custos,
    ...
    while (!finished) {
        *d_finished = 1;
        sssp_kernel<<<numBlocks, blockSize>>>(numEdges, origens, destinos, custos,
                                              dist, preNode, d_finished);
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] sssp cuda N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("sssp", "cuda", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "sssp cuda %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL sssp cuda N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio sssp cuda hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

...
/* Bitonic sort iterativo, ordenacao crescente. Loops k e j sequenciais,
   apenas o loop de indices i e paralelizado com OpenMP */
static void bitonic_sort_openmp(int32_t *arr, int n) {
    for (int k = 2; k <= n; k <<= 1) {
...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 1; iter <= iteracoes; iter++) {
        ...
        printf("bitonic_sort|openmp|aleatorio|%d|%d|%s|%d|%dx1\n",
               n, iter, tempo_str, corretude, omp_get_max_threads());
    }

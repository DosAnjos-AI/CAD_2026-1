...
CSV_PATH = "results/mx350/CAD_mx350_corrigido.csv"
...
for algo, ordem in ORDENS.items():
    ...
    ax.set_title(algo.capitalize(), fontsize=13, fontweight="bold")
    ax.set_ylim(bottom=0)

    patches = [mpatches.Patch(color=CORES[a], label=LABELS[a]) for a in APIS]

...
CSV_PATH = "results/jetson_agx_orin/CAD_jetson_agx_orin.csv"
OUT_DIR = "results/graficos/jetson"
...
df = pd.read_csv(CSV_PATH, sep="|", decimal=",", keep_default_na=False, na_values=[])
df["tamanho"]       = df["tamanho"].astype(str)
df["tempo_total_s"] = pd.to_numeric(df["tempo_total_s"], errors="coerce")
...
for algo, ordem in ORDENS.items():
    ...
    ax.set_title(f"{algo.capitalize()} — Jetson AGX Orin", fontsize=13, fontweight="bold")
    ...
    out_path = os.path.join(OUT_DIR, f"tempo_{algo}.png")

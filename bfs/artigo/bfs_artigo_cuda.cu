/* bfs_artigo_cuda.cu
 * Implementacao fiel ao Algoritmo 7 do apendice de Nogueira et al. (SSCAD 2024).
 * Kernel queue-based: cada thread processa um no da fila atual.
 * Memoria convencional (cudaMalloc + cudaMemcpy).
 * Uso: ./bfs_artigo_cuda [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void grafo_gpu_alocar(int N, int E,
    ...
    cudaMalloc(d_adj,    E * sizeof(int));
    cudaMalloc(d_offset, N * sizeof(int));
    cudaMalloc(d_size,   N * sizeof(int));
    cudaMemcpy(*d_adj,    h_adj,    E * sizeof(int), cudaMemcpyHostToDevice);
...
/* ---------- kernel BFS (Algoritmo 7) ---------- */

__global__ void bfs_kernel(int nivel, int *adjacencyList, int *edgesOffset,
                            int *edgesSize, int *distance,
                            int queueSize, int *nextQueueSize,
                            int *currentQueue, int *nextQueue) {
    int thid = blockIdx.x * blockDim.x + threadIdx.x;
    if (thid >= queueSize) return;

    int u = currentQueue[thid];
    for (int i = edgesOffset[u]; i < edgesOffset[u] + edgesSize[u]; i++) {
        int v = adjacencyList[i];
        if (distance[v] == INT_MAX
                && atomicMin(&distance[v], nivel + 1) == INT_MAX) {
...
static void bfs_cuda(int N, int *d_adj, int *d_offset, int *d_size, int *h_dist) {
    int *d_dist, *d_currentQueue, *d_nextQueue, *d_nextQueueSize;

    cudaMalloc(&d_dist,          N * sizeof(int));
    cudaMalloc(&d_currentQueue,  N * sizeof(int));
    cudaMalloc(&d_nextQueue,     N * sizeof(int));
    cudaMalloc(&d_nextQueueSize, sizeof(int));

    cudaMemcpy(d_dist, h_dist, N * sizeof(int), cudaMemcpyHostToDevice);

    int root = 0;
    cudaMemcpy(d_currentQueue, &root, sizeof(int), cudaMemcpyHostToDevice);
    ...
    while (queueSize != 0) {
        int numBlocks = (queueSize + blockSize - 1) / blockSize;
        bfs_kernel<<<numBlocks, blockSize>>>(nivel, d_adj, d_offset, d_size, d_dist,
                                             queueSize, d_nextQueueSize,
                                             d_currentQueue, d_nextQueue);
        cudaDeviceSynchronize();
        nivel++;
        cudaMemcpy(&queueSize, d_nextQueueSize, sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemset(d_nextQueueSize, 0, sizeof(int));
        int *tmp = d_currentQueue; d_currentQueue = d_nextQueue; d_nextQueue = tmp;
    }
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        bfs_cuda(N, d_adj, d_offset, d_size, dist);
        ...
        printf("[TESTE] bfs cuda N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
    ...
    free(adj); free(offset); free(size); free(dist); free(dist_ref);
}
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("bfs", "cuda", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "bfs cuda %s N=%d iter=%d tempo=%.6fs corretude=%s",
                         hardware_label, N, iter, tempo_medio, corr_str);
                log_msg("INFO", msg);
            }
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg), "FAIL bfs cuda N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio bfs cuda hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

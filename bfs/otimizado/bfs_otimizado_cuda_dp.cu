/* bfs_otimizado_cuda_dp.cu
 * Versao otimizada do Algoritmo 3 de Nogueira et al. (SSCAD 2024).
 * OPT-1: atomicExch por thread em vez de blkChanged + atomicOr.
 * OPT-2: kernel secundario lancado apenas se edgesSize[0] > LIMIAR_ADJ.
 * Memoria unificada (cudaMallocManaged). Compilar com -rdc=true -lcudadevrt.
 * Uso: ./bfs_otimizado_cuda_dp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- kernels BFS otimizados ---------- */

/* OPT-1: cada thread escreve atomicExch(changed,1) diretamente ao detectar
 * mudanca, eliminando blkChanged em memoria compartilhada e atomicOr. */
    ...
    if (thid < N && distance[thid] != INT_MAX) {
        ...
        for (int i = edgesOffset[thid];
            ...
            if (atomicMin(&distance[v], d + 1) > d + 1)
                atomicExch(changed, 1);  /* OPT-1 */
        }
    }
}

/* OPT-1: atomicExch por thread; OPT-2: kernel secundario lancado apenas se
 * edgesSize[0] > LIMIAR_ADJ (no com poucas adjacencias e tratado na proxima
 * iteracao pelo kernel principal, sem perda de corretude). */
__global__ void bfs_kernel_dp(int *distance, int nivel, int *adjacencyList,
    ...
    __syncthreads();
    /* OPT-2: bloco 0, thread 0 lanca secundario apenas se no 0 tem adjacencias
     * suficientes (thid == 0 quando blockIdx.x == 0 && threadIdx.x == 0) */
    if (threadIdx.x == 0 && blockIdx.x == 0 && edgesSize[thid] > LIMIAR_ADJ) {
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] bfs cuda_dp otimizado N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("bfs", "cuda_dp", "otimizado", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "bfs cuda_dp otimizado %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL bfs cuda_dp otimizado N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
...

    {
        char msg[128];
        snprintf(msg, sizeof(msg),
                 "inicio bfs cuda_dp otimizado hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

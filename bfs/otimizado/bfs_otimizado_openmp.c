/* bfs_otimizado_openmp.c
 * BFS OpenMP com fila explicita (OPT-1): substitui varredura O(V) por
 * processamento da fila atual — O(fila) por nivel.
 * schedule(dynamic,64) + #pragma omp critical com double-check.
 * Uso: ./bfs_otimizado_openmp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- bfs otimizado — OPT-1: fila explicita ---------- */

static void bfs_openmp(int N, int *adj, int *offset, int *size, int *distancia) {
    for (int i = 0; i < N; i++) distancia[i] = INT_MAX;
    distancia[0] = 0;

    int *fila_atual   = (int *)malloc(N * sizeof(int));
    int *fila_proxima = (int *)malloc(N * sizeof(int));
    int  tam_atual    = 1;
    int  tam_proximo  = 0;
    ...
    while (tam_atual > 0) {
        ...
        #pragma omp parallel for schedule(dynamic, 64) shared(tam_proximo, fila_proxima)
        for (int i = 0; i < tam_atual; i++) {
            ...
            for (int j = offset[u]; j < offset[u] + size[u]; j++) {
                ...
                if (distancia[v] == INT_MAX) {
                    #pragma omp critical
                    {
                        /* double-check para evitar insercao duplicada na fila */
                        if (distancia[v] == INT_MAX) {
                            distancia[v] = distancia[u] + 1;
                            fila_proxima[tam_proximo++] = v;
                        }
                    }
                }
            }
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] bfs openmp otimizado N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("bfs", "openmp", "otimizado", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "bfs openmp otimizado %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL bfs openmp otimizado N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
...

    {
        char msg[128];
        snprintf(msg, sizeof(msg),
                 "inicio bfs openmp otimizado hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

/* bfs_otimizado_cuda.cu
 * Versao otimizada do BFS CUDA baseada em rafalk342/bfs-cuda (kernel queueBfs).
 * OPT-1: queueBfs processa apenas vertices da fila atual — O(fila) por nivel.
 * OPT-2: numBlocks dinamico por queueSize (consequencia da OPT-1), 128 threads/bloco.
 * nextQueueSize em memoria unificada (cudaMallocManaged) — sem cudaMemcpy por iteracao.
 * Uso: ./bfs_otimizado_cuda [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- kernel BFS: processa apenas vertices da fila atual (rafalk342/bfs-cuda) ---------- */
...
/* ---------- bfs cuda otimizado: execucao ---------- */

static void bfs_cuda(int N, int *d_adj, int *d_offset, int *d_size, int *h_dist) {
    int *d_dist, *d_currentQueue, *d_nextQueue, *d_parent;
    int *nextQueueSize;
    ...
    *nextQueueSize = 0;
    ...
    while (queueSize > 0) {
        int numBlocks = (queueSize + numThreadsPerBlock - 1) / numThreadsPerBlock;
        queueBfs<<<numBlocks, numThreadsPerBlock>>>(
            level, d_adj, d_offset, d_size, d_dist, d_parent,
            queueSize, nextQueueSize, d_currentQueue, d_nextQueue);
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] bfs cuda otimizado N=%d E=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, E, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tam; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("bfs", "cuda", "otimizado", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "bfs cuda otimizado %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL bfs cuda otimizado N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio bfs cuda otimizado hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

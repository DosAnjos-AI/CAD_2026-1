...
__global__ void mergesort_kernel(int *arr, int *aux,
    ...
    if (num_elementos > threshold) {
        /* Copia secao de arr para aux antes de lancar kernel filho */
        for (int i = inicio; i <= fim; i++)
            aux[i] = arr[i];
...
int main(int argc, char *argv[]) {
    ...
    /* Gera vetor aleatorio uma vez — mesmo dado em todas as execucoes */
    for (int i = 0; i < n; i++)
        h_arr[i] = rand();

    /* Warm-up run 1: aquece a GPU e verifica corretude antes de medir */
    {
    ...
    /* Warm-up runs 2 e 3: sem validacao, apenas aquecimento adicional da GPU */
    for (int w = 0; w < 2; w++) {
        cudaMemcpy(d_arr, h_arr, n * sizeof(int), cudaMemcpyHostToDevice);
        for (int tamanho_atual = 1; tamanho_atual < n; tamanho_atual *= 2) {
            int largura   = tamanho_atual * 2;
            ...
            cudaError_t err = cudaGetLastError();
            if (err != cudaSuccess)
                printf("ERRO no kernel pai (warm-up): %s\n", cudaGetErrorString(err));
        }
    }
    ...
    for (int it = 0; it < ITERACOES; it++) {
        ...
        printf("mergesort,cuda_dp_otimizado,%d,%d,%d,%.6f,%.6f,%.6f,%s\n",
               n, it + 1, EXECUCOES, soma_iter, energia, 0.0, corretude);
        fflush(stdout);
    }

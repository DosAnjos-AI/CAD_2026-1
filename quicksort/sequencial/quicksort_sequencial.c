/* quicksort_sequencial.c
 * Versao sequencial pura — baseline sem paralelismo.
 * Algoritmo 4 do apendice de Nogueira et al. (SSCAD 2024), sem tasks OpenMP.
 * Uso: ./quicksort_sequencial [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- algoritmo — Algoritmo 4 do apendice, sem tasks ---------- */
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] quicksort sequencial N=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, exec, tempo, corr);
    }
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            for (int rep = 0; rep < N_REPS; rep++) {
                ...
                if (rep == 0 && metodo_energia == ENERGIA_RAPL)
                    rapl_antes = rapl_ler();
            ...
            char ecpu[32] = "NA";
            if (metodo_energia == ENERGIA_RAPL
            ...
            csv_append("quicksort", "sequencial", "sequencial", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "quicksort sequencial %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL quicksort sequencial N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio quicksort sequencial hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

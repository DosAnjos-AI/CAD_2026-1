/* quicksort_artigo_cuda_dp.cu
 * Implementacao fiel ao Algoritmo 2 do artigo de Nogueira et al. (SSCAD 2024).
 * Base: NVIDIA/cuda-samples cdpSimpleQuicksort.
 * CUDA Dynamic Parallelism: kernel recursivo com fallback para selection_sort.
 ...
 * Uso: ./quicksort_artigo_cuda_dp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- algoritmo — base: cdpSimpleQuicksort (NVIDIA/cuda-samples) ---------- */

/* fallback O(n^2) — Algoritmo 2 do artigo */
__device__ static void selection_sort(int *data, int left, int right) {
    for (int i = left; i <= right; i++) {
        int min_idx = i;
        for (int j = i + 1; j <= right; j++)
            if (data[j] < data[min_idx]) min_idx = j;
        if (min_idx != i) {
            int tmp = data[i]; data[i] = data[min_idx]; data[min_idx] = tmp;
        }
    }
}
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] quicksort cuda_dp N=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, exec, tempo, corr);
    }
    {
        char egpu_t[32] = "NA";
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("quicksort", "cuda_dp", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "quicksort cuda_dp %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL quicksort cuda_dp N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio quicksort cuda_dp hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

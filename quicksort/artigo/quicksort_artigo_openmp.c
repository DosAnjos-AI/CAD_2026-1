/* quicksort_artigo_openmp.c
 * Implementacao fiel ao Algoritmo 4 do apendice de Nogueira et al. (SSCAD 2024).
 * Top-down com #pragma omp task; sem threshold, sem taskwait explicito.
 * Uso: ./quicksort_artigo_openmp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ---------- algoritmo — Algoritmo 4 do apendice ---------- */
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] quicksort openmp N=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, exec, tempo, corr);
    }
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("quicksort", "openmp", "artigo", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "quicksort openmp %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL quicksort openmp N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio quicksort openmp hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

/* quicksort_artigo_cuda.cu
 * Implementacao fiel ao Algoritmo 3 do apendice de Nogueira et al. (SSCAD 2024).
 * Base: repositorio gongminaaa/GPU_Parallel_Computing.
 * Loop ping-pong entre list1/list2; kernels quickSort e arrangeQueueAndPivot.
 * Memoria unificada (cudaMallocManaged). Tipo int.
 * Uso: ./quicksort_artigo_cuda [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
__global__ void arrangeQueueAndPivot(int *new_pivot_queue, int *old_pivot_queue,
    ...
    /* sem syncthreads: cada thread acessa apenas seu proprio indice */
...
#if PICK_PIVOT
    /* selecionar melhor pivo (mediana de 3 candidatos) e colocar na primeira posicao */
...
/* reinicializa todos os buffers para o estado inicial de uma execucao */
...
/* executa o loop ping-pong e retorna ponteiro para o buffer com o resultado */
static int *executar_quicksort(int *list1, int *list2,
                                int *q, int *q_copy, int *q2, int *q_copy2,
                                int *pivot_arr, bool *done,
                                int N, int blockSize, int numBlocks) {
    int count = 0;
    while (!(*done)) {
        *done = true;
        if (count % 2 == 0) {
            quickSort<<<numBlocks, blockSize>>>(N, list1, list2, q, q_copy, pivot_arr);
            cudaDeviceSynchronize();
            arrangeQueueAndPivot<<<numBlocks, blockSize>>>(q2, q, N, list2, pivot_arr,
                                                            q_copy2, q_copy, done);
            cudaDeviceSynchronize();
        } else {
            quickSort<<<numBlocks, blockSize>>>(N, list2, list1, q2, q_copy2, pivot_arr);
            cudaDeviceSynchronize();
            arrangeQueueAndPivot<<<numBlocks, blockSize>>>(q, q2, N, list1, pivot_arr,
                                                            q_copy, q_copy2, done);
            cudaDeviceSynchronize();
        }
        count++;
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        /* warmup */
        resetar_estado(list1, original, pivot_arr, q, q2, q_copy, q_copy2, done, N);
        executar_quicksort(list1, list2, q, q_copy, q2, q_copy2,
                           pivot_arr, done, N, blockSize, numBlocks);
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("quicksort", "cuda", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "quicksort cuda %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL quicksort cuda N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio quicksort cuda hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

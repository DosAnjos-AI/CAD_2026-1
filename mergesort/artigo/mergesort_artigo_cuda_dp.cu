/* mergesort_artigo_cuda_dp.cu
 * Implementacao fiel ao Algoritmo 1 do artigo de Nogueira et al. (SSCAD 2024).
 * CUDA Dynamic Parallelism: kernel principal lanca Kernel_binary_search quando
 * o subproblema e suficientemente grande; caso contrario usa insertion_sort.
 * Memoria unificada (cudaMallocManaged). Compilar com -rdc=true -lcudadevrt.
 * Uso: ./mergesort_artigo_cuda_dp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* insertion sort O(n^2) in-place em data[inicio..fim] (fim inclusivo)
 * usado quando num_threads <= 16384 ou >= 1024*1024 */
__device__ static void insertion_sort(int *data, int inicio, int fim) {
    for (int i = inicio + 1; i <= fim; i++) {
        int chave = data[i];
        int j     = i - 1;
        while (j >= inicio && data[j] > chave) {
            data[j + 1] = data[j];
            j--;
        }
        data[j + 1] = chave;
...
/* merge paralelo com busca binaria — lancado via CUDA DP
 * cada thread calcula o elemento da posicao 'idx' no resultado do merge
 * de data[inicio..meio] com data[meio+1..fim]; resultado gravado em buf */
__global__ void Kernel_binary_search(int *data, int *buf,
                                      int inicio, int meio, int fim) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int La  = meio - inicio + 1;     /* tamanho da metade esquerda */
    int Ra  = fim  - meio;           /* tamanho da metade direita */
    int n   = La + Ra;

    if (idx >= n) return;

    int p  = idx;
    /* intervalo valido de il: [max(0, p-Ra), min(p, La)] */
    int lo = (p > Ra)  ? p - Ra : 0;
    int hi = (p < La)  ? p      : La;

    /* busca binaria: maior il tal que L[il-1] <= R[p-il] */
    while (lo < hi) {
        int mid   = lo + (hi - lo) / 2;
        int r_pos = p - mid - 1;       /* indice em R do ultimo elemento antes de p */
        if (data[inicio + mid] <= data[meio + 1 + r_pos])
            lo = mid + 1;
        else
            hi = mid;
    }
    ...
    int il    = lo;
    int ir    = p - il;
    int val_l = (il < La) ? data[inicio + il]   : IMAX;
    int val_r = (ir < Ra) ? data[meio  + 1 + ir] : IMAX;

    buf[inicio + p] = (val_l <= val_r) ? val_l : val_r;
}

/* kernel principal — Algoritmo 1 do artigo
 * cada thread trata um par de subarrays [inicio, meio] e [meio+1, fim] */
__global__ void mergesort_dp_kernel(int *data, int *buf,
                                     int size, int tamanho_atual) {
    int largura = tamanho_atual * 2;
    int idx     = blockIdx.x * blockDim.x + threadIdx.x;
    int inicio  = largura * idx;

    if (inicio >= size) return;

    int meio = inicio + tamanho_atual - 1;
    int fim  = min(inicio + largura - 1, size - 1);

    /* nada a mesclar: metade direita esta alem do vetor */
    if (meio >= size) return;

    int num_threads = fim - inicio + 1;

    if (num_threads > 16384 && num_threads < 1024 * 1024) {
        int numTPB    = 1024;
        int numBlocks = (num_threads + numTPB - 1) / numTPB;
        Kernel_binary_search<<<numBlocks, numTPB>>>(data, buf, inicio, meio, fim);
        cudaDeviceSynchronize();
        /* copiar resultado de buf de volta para data */
        for (int k = inicio; k <= fim; k++) data[k] = buf[k];
    } else {
        insertion_sort(data, inicio, fim);
    }
}
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        for (int tamanho_atual = 1; tamanho_atual < N; tamanho_atual *= 2) {
            ...
            int numTPB    = 1024;
            int numBlocks = (num_sorts + numTPB - 1) / numTPB;
            mergesort_dp_kernel<<<numBlocks, numTPB>>>(data, buf, N, tamanho_atual);
            cudaDeviceSynchronize();
        }

        const char *corr = verificar_corretude(data, N) ? "OK" : "FAIL";
        printf("[TESTE] mergesort cuda_dp N=%d exec=%d corretude=%s\n", N, exec, corr);
    }
    {
        char egpu_t[32] = "NA";
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            for (int rep = 0; rep < N_REPS; rep++) {
                ...
                if (rep == 0)
                    corr_ok = verificar_corretude(data, N);
            }
            ...
            csv_append("mergesort", "cuda_dp", "artigo", hardware_label,
                       N, iter, tempo_medio, egpu, ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "mergesort cuda_dp %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corr_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL mergesort cuda_dp N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio mergesort cuda_dp hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

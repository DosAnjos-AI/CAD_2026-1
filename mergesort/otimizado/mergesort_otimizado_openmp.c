/* mergesort_otimizado_openmp.c
 * Versao otimizada do Algoritmo 6 de Nogueira et al. (SSCAD 2024).
 * OPT-1: conquista via merge sequencial O(n) no lugar de selection sort O(n^2).
 * OPT-2: threshold LIMIAR para suprimir criacao de tasks em subproblemas pequenos.
 * Uso: ./mergesort_otimizado_openmp [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* OPT-1: merge linear O(n) no lugar do selection sort O(n^2) do artigo */
static void merge_sequencial(int *vetor, int *tmp, int inicio, int meio, int fim) {
    int i = inicio, j = meio + 1, k = inicio;
    while (i <= meio && j <= fim)
        tmp[k++] = (vetor[i] <= vetor[j]) ? vetor[i++] : vetor[j++];
    while (i <= meio) tmp[k++] = vetor[i++];
    while (j <= fim)  tmp[k++] = vetor[j++];
    for (int k2 = inicio; k2 <= fim; k2++) vetor[k2] = tmp[k2];
}

/* OPT-2: cláusula if(tamanho > LIMIAR) suprime tasks para subproblemas pequenos */
static void mergesort(int *vetor, int inicio, int fim, int *tmp) {
    if (inicio < fim) {
        int separar = (inicio + fim) / 2;
        #pragma omp task if(separar - inicio > LIMIAR)
        mergesort(vetor, inicio, separar, tmp);
        #pragma omp task if(fim - (separar + 1) > LIMIAR)
        mergesort(vetor, separar + 1, fim, tmp);
        #pragma omp taskwait
        merge_sequencial(vetor, tmp, inicio, separar, fim);
    }
}
...
static void modo_teste(void) {
    ...
    for (int exec = 1; exec <= 2; exec++) {
        ...
        printf("[TESTE] mergesort openmp otimizado N=%d exec=%d tempo=%.6fs corretude=%s\n",
               N, exec, tempo, corr);
    }
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("mergesort", "openmp", "otimizado", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "mergesort openmp otimizado %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL mergesort openmp otimizado N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio mergesort openmp otimizado hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

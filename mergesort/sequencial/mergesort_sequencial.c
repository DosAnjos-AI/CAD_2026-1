/* mergesort_sequencial.c
 * Mergesort top-down recursivo sequencial puro (sem OpenMP, sem CUDA).
 * Serve como baseline para calculo de speedup.
 * Uso: ./mergesort_sequencial [--teste]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void merge(int *vetor, int *tmp, int inicio, int meio, int fim) {
...
static void mergesort(int *vetor, int *tmp, int inicio, int fim) {
    if (inicio < fim) {
        int meio = (inicio + fim) / 2;
        mergesort(vetor, tmp, inicio, meio);
        mergesort(vetor, tmp, meio + 1, fim);
        merge(vetor, tmp, inicio, meio, fim);
    }
}
...
static void modo_benchmark(void) {
    ...
    for (int ti = 0; ti < n_tamanhos; ti++) {
        ...
        for (int iter = 1; iter <= N_ITER; iter++) {
            ...
            csv_append("mergesort", "sequencial", "sequencial", hardware_label,
                       N, iter, tempo_medio, "NA", ecpu, corr_str);

            {
                char msg[256];
                snprintf(msg, sizeof(msg),
                         "mergesort sequencial %s N=%d iter=%d tempo=%.6fs corretude=%s",
            ...
            if (!corretude_ok) {
                char msg[128];
                snprintf(msg, sizeof(msg),
                         "FAIL mergesort sequencial N=%d iter=%d", N, iter);
                log_msg("ERROR", msg);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "inicio mergesort sequencial hardware=%s", hardware_label);
        log_msg("INFO", msg);
    }

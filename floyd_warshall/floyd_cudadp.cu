...
/* Um thread por par (i, j): atualiza dist[i][j] usando o vertice intermediario k.
   Identico ao kernel de floyd_cuda.cu, agora lancado como kernel filho */
...
/* Kernel pai: controla o loop de k no device. Para cada k, lanca o kernel
   filho fw_kernel e agenda a proxima iteracao via cudaStreamTailLaunch,
   que so comeca apos o filho atual terminar */
__global__ void fw_dp_launcher(int32_t *dist, int v, int k) {
    if (k >= v) return;
    ...
    fw_kernel<<<blocks, threads>>>(dist, v, k);
    fw_dp_launcher<<<1, 1, 0, cudaStreamTailLaunch>>>(dist, v, k + 1);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 1; iter <= iteracoes; iter++) {
        ...
        printf("floyd_warshall|cudadp|aleatorio|%d|%d|%s|%d|1x1_dp\n",
